...
| `MainLayout.tsx` | 无任何响应式逻辑 | 布局不区分手机/PC |
| `DashboardPage.tsx` | `gridTemplateColumns: 'repeat(4, 1fr)'` 硬编码 4 列 | 手机端单格 65px，内容溢出 |
| `QueryLogPage.tsx` | 原生 `<table>` 各列宽固定，合计 ≈ 912px | 手机端水平滚动无法使用 |
| `SteeringListPage.tsx` | flex 行内固定宽度列（100/120/140/280px） | 手机端列宽溢出 |
| 所有 Modal | 未设置响应宽度限制 | 手机端弹窗超出屏幕 |

---

## 设备判断方式

**使用 User Agent 检测，而非 viewport 尺寸。**
...
- **手机设备**（UA 匹配上述正则）→ 手机布局（Drawer 导航、卡片列表、紧凑 Header）
- **其他设备**（PC、桌面浏览器缩窗口等）→ PC 布局（固定 Sidebar、完整表格）
- 不使用 `useBreakpoint()` 或 `window.innerWidth` 判断
...
- **Given** 手机设备，Drawer 关闭
  **When** 点击汉堡图标
  **Then** Sidebar 以 Drawer 形式从左侧滑入，宽度 ≤ 280px
- **Given** Drawer 已打开
  **When** 点击遮罩层或 Drawer 内任意菜单项
  **Then** Drawer 关闭
- **Given** 非手机设备（PC/桌面浏览器）
  **When** 页面加载
  **Then** 还原为固定 Sidebar，汉堡图标不可见
...
- **Given** 手机设备（UA 匹配）
  **When** 任意页面加载
  **Then** Header padding 收窄为 `0 12px`，头像可点击，汉堡图标可见
- **Given** 非手机设备（PC/桌面浏览器）
  **When** 任意页面加载
  **Then** Header 还原为完整 PC 样式（padding `0 24px`）
...
IT-001: 手机设备（UA 匹配）下，Sidebar 不可见，汉堡图标可见
IT-002: 手机设备下，点击汉堡图标，Drawer 从左侧滑入
IT-003: Drawer 打开时，点击遮罩层，Drawer 关闭
IT-004: 非手机设备（PC）下，Sidebar 正常显示，汉堡图标不可见
IT-005: 手机设备下，QueryLogPage 显示卡片列表而非 table
IT-006: 手机设备下，DashboardPage 统计卡片为 1 列
IT-007: 手机设备下，任意 Modal 宽度不超过屏幕宽度
IT-008: 非手机设备（PC）下，所有页面恢复 PC 布局
