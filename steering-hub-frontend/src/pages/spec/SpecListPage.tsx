...
import { qualityService, type SpecQuality } from '../../services/searchService';
...
export default function SpecListPage() {
  ...
  const [data, setData] = useState<{ records: Spec[]; total: number; pages: number } | null>(null);
  const [loading, setLoading] = useState(true);
  const [qualityData, setQualityData] = useState<Record<number, SpecQuality | null>>({});
  ...
  useEffect(() => {
    ...
    setActions(
      <Button type="primary" icon={<Plus size={14} />} onClick={() => navigate('/specs/new')}>
        新建规范
      </Button>
    );
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const result = await specService.page({ current: page + 1, size: 10, status: status || undefined, keyword });
        setData(result);
        // Load quality data
  ...
  const handleDelete = async () => {
    ...
    try {
      await specService.delete(deleteId);
  ...
  const handleReview = async (id: number, action: 'submit' | 'activate' | 'deprecate') => {
    try {
      await specService.review(id, action);
            ...
            {data?.records?.map((record) => (
              <Flex key={record.id} align="center" style={{ padding: '10px 20px', borderBottom: '1px solid #27273a' }}>
                ...
                <Flex gap={4} style={{ width: 200 }}>
                  <Button type="link" size="small" onClick={() => navigate(`/specs/${record.id}/edit`)} style={{ color: '#a1a1aa', fontSize: 12 }}>编辑</Button>
                  {record.status === 'draft' && (
